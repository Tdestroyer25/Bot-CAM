...
Recuerda siempre seguir las reglas y mantener una formalidad respetuosa`
    ...
    "body": 'Facil y Rapido',  
    "thumbnail": fs.readFileSync('./media/imagen/Otaku1.jpg'),
...
if (test.includes(`grupo`)){
  ...
  *-Te invito a unirte a Nuestro grupo de WhatsApp y Facebook donde:-*
  
  💭:Hacemos dinamicas, Concursos y Sorteo de todo tipo de Cuenta. 
  
  💭:Tus Amigos pueden ser Parte de nuestra Comunidad 
  
  💭:Estamos Activos 24/7
  
  Grupo 1 : 
  _https://chat.whatsapp.com/JW55BQ6uSne3UPECWOUsUu_
  
  Grupo 2 : 
  _https://www.facebook.com/groups/244228800799659/_`, MessageType.text, {quoted: choute})
  addFilter(from)
    addLevelingLevel(sender, 5)	}}
...
if (choute.message.listResponseMessage){
  ...
  `${texto}\n${allmenu}`
    ...
    imageMsg = (await leo.prepareMessageMedia(fs.readFileSync(`./media/imagen/Zenitsu.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/imagen/fake.jpg`)})).imageMessage
    buttonsMessage = {
    contentText: `${txtt}`,
...
addLevelingLevel(sender, 5)	}}
  ...
  if (choute.message.listResponseMessage){
    ...
    leo.sendMessage(from, fs.readFileSync('./media/imagen/Papuso2.jpeg') , MessageType.image, {caption: `${texto}\n${multimedia}`, quoted: choute,  contextInfo: { mentionedJid: [sender,Oowner]} })  
    addFilter(from)
    addLevelingLevel(sender, 5)	}}
  ...
  if (choute.message.listResponseMessage){
    ...
    if (!isRegister) return reply(baby.only.usrReg)
    uptime = process.uptime()
    leo.sendMessage(from, fs.readFileSync('./media/imagen/Papuso1.jpeg'), MessageType.image, {caption: `${texto}\n${entrete}`,quoted: choute,  contextInfo: { mentionedJid: [sender,Oowner]} })  
    addFilter(from)
    addLevelingLevel(sender, 5)	}}
    //Logo
  if (choute.message.listResponseMessage){
...
case 'reg':   
            ...
            if (edad > 50) return reply(`Pero-\n*Tienes mas de  50 años, no te puedes registrar, mejor cuida tus nietos :D*`)
